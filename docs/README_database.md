...
L'accès à la base de données se fait via `dbService` (instance de `DatabaseService`) qui abstrait les opérations SQL :

```typescript
// Exemples d'opérations réelles
const user = await dbService.getUserById(userId);
const stats = await dbService.getUserStats(userId);
await dbService.updateUserProfile(userId, { username, avatar_url });
